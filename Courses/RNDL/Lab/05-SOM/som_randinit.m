...
for i = 1:dim,
  ...
  else ma = max(D(inds,i)); mi = min(D(inds,i));  %individua massimo e minimao tra tutti valori del training set non infiniti e non scorretti (ma qs non dovrebbero essere prodotti da rand!)
  end
  sMap.codebook(:,i) = (ma - mi) * sMap.codebook(:,i) + mi; %normalizza i pesi secondo il valore dei training set (quanto meno range per ogni attributo): in questo modo i pesi sono uniformemente distribuiti in quel range 
end
