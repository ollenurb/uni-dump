# Image Enhancement (Dominio Spaziale)
...
all'operatore di *Sobel*. L'operatore di Prewitt per le linee orizzontali è
implementato nel modo seguente (quello per le linee verticali ne è semplicemente
una rotazione di 90 gradi in senso antiorario)
$$
\begin{bmatrix}
-1 & -1 & -1 \\ 
0 & 0 & 0 \\ 
...
derivate direzionali (prima si calcola poi si fa la somma).
...
opportunamente per ognuno di esso queste statistiche. Il processing verrà
effettuato solo se i seguenti vincoli sono soddisfatti:
...
* Se $\sigma_S \leq k_2 \sigma_G$, in modo da considerare solo i pixel che non
  sono già in una zona ad altro constrasto;
* Se $k_1 \sigma_G \leq \sigma_S$ (ovviamente $k_1 < k_2$) in modo tale da
