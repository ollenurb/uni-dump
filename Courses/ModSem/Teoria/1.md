...
* L'idea era quella di aggiungere capacità espressiva alle reti semantiche
...
* La struttura di un frame è quindi la seguente:
    ...
    * **Procedure per il calcolo di valori**: permettono di calcolare
      ...
      specifiche;
    * **Valori predefiniti**: valori impostati di default.
...
* I motori inferenziali hanno dei meccanismi per evitare di applicare la
...
* Per fare ciò, queste funzionalità sono eseguite esplicitamente dall'utente,
  utilizzando un linguaggio apposito;
* Un nodo in SNePs può essere:
    * **Molecolare** ($M_i$): rappresenta delle proposizioni, regole di
      ragionamento o individui. Ha solo archi uscenti.
    * **Base** ($B_i$): rappresenta individui che hanno certe caratteristiche
      a cui non si vuole dare però un nome. Non ha archi uscenti.
    * **Variabile**: rappresenta individui o proposizioni. Corrisponde alle
      variabili logiche. Non ha archi uscenti.
...
* L'interrogazione avviene tramite il comando `find`
    * es. `(find class elephant)` trova tutti i nodi che sono connessi al nodo
      elephant
...
* Le inferenze in questo sistema avvengono in tre modi differenti:
    1. Riduzione:
        * Un nodo proposizionale `M1` implica un nodo proposizionale `M2` che ha
          un subset o un superset degli archi di `M1`.
