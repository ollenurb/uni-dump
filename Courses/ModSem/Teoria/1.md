...
* Per impiegare queste tecnologie per rappresentare dati strutturati si seguono
  ...
  2. Si utilizzano i vocabolari condivisi per rappresentare i dati,
     identificando vocabolari ed entita' descritte attraverso gli IRI.
        * Corrisponde semplicemente all'utilizzo della conoscenza asiomatizzata
          al passo precedente.
* Il risultato di queste due operazioni e' un *grafo* di conoscenza, che puo'
...
* Il ragionamento in una rete semantica corrisponde a **seguire un percorso**
  all'interno del grafo.
    * es. Se voglio sapere di che colore e' il blocco collegato sul tavolo,
...
* Un'altra problematica delle reti semantiche e' data dal fatto che le relazioni
  sono solo binarie, per cui per rappresentare delle relazioni che convolgono
  piu' di 2 entita' viene fatto introducendo un nodo che rappresenta la
  relazione (o evento) e connettendolo a tutte le entita' che fanno parte della
  relazione.
