...
* Centrale a RDF è il concetto di *risorsa* che può essere un servizio,
  documento, pagina web, ecc.. (in generale, qualsiasi cosa identificabile per
  mezzo di un IRI)
...
> oggetto. Il soggetto e l'oggetto possono essere **IRI** o **blank nodes**.
> L'oggetto può essere anche un **letterale**, mentre il predicato può essere
> solo un **IRI**.
...
* Il punto di RDF è quello di riutilizzare vocabolari standard che rappresentano
  termini, invece che ri-definirli sempre da 0
...
* RDF di per sè è una *sintassi astratta* che deve poi essere serializzata in
  ...
  serializzazione, ognuno con i suoi pro e contro:
    * Turtle (preferito per concisione e leggibilità)
    * RDF/XML
    * N-triples
    * Json LD
    * N3
...
* Insieme di vocabolari con il fine di rappresentare dati strutturati quali
  pagine web, email e altro
