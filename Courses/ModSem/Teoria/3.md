...
* Le ontologie formali fissano un particolare formalismo di rappresentazione,
  con la fintalità di condividere una concettualizzazione comune tra individui,
...
* **CYC**:
    * Un esempio di ontologia fondazionale è **CYC** (OpenCYC) una base di
    ...
    * Il motore inferenziale di CYC è in grado di effettuare deduzione logica e i
...
* **Wordnet**:
    ...
    * Suddivide le parole in *synsets*, cioè insiemi di parole che condividono lo
      stesso significato
* **DbPedia**:
    * È una versione semantica di Wikipedia che è stat costruita per mezzo di un
...
* L'*Ontology Web Language* (OWL) è un linguaggio atto a creare onologie per il
...
* Il ragionamento avviene su più livelli:
    ...
    * **Inserimento di individui**
        * Il reasoner assegna un particolare individuo ad una classe
          automaticamente sulla base delle sue proproietà/caratteristiche
    ...
    * **Classificazione Automatica**
        * È un caso specifico dell'inseriemento, essenzialmente è in grado di
          capire se un individuo che fa parte di una classe più generale fa in
          realtà parte di una sottoclasse classe più specifica
...
* Alcune di queste ontologie più affermate sono *OntoClean* e *Neon*
...
* Le metaproprietà menzionate sono 4, ognuna identiticata da una lettera:
...
* La metodologia consiste nei seguenti passaggi:
    1. Si analizzano le meta-proprietà delle classi, da cui emergeranno
        ...
        * Ogni classe viene annotata con le meta-proprietà
...
* Un agente prende parte ad una attività attraverso un ruolo in modo tale che
...
* Le attività definiscono come le entità vengono ad esistere e come i loro
  attributi cambiano per diventare nuove entità
